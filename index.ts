// =================================================================================
//  Project: toolbaz-2api (Bun Version)
//  Version: 1.9.4-bun (Ported by CezDev)
//  Date: 2025-11-24
//
//  [Bun Port Notes]
//  - Replaced Cloudflare 'export default' with 'Bun.serve'
//  - Removed ctx.waitUntil (Bun handles async streams natively)
//  - Renamed API_MASTER_KEY to API_KEY
// =================================================================================

// --- [Part 1: Core Configuration] ---
const CONFIG = {
  PROJECT_NAME: "toolbaz-2api-bun",
  PROJECT_VERSION: "1.9.4-bun",
  
  // Update: Changed to API_KEY, allows override via environment variable
...
class DebugLogger {
  ...
  add(step, type, data) {
    ...
    // Optional: Log to server console in Bun for easier debugging
    // console.log(`[${entry.time}] [${type}] ${step}`);
    return entry;
  }
...
Bun.serve({
  ...
  async fetch(request) {
    ...
    try {
        ...
        if (url.pathname === '/v1/images/generations') return handleImageGenerations(request);
        if (url.pathname === '/v1/models') return handleModelsRequest();

        return createErrorResponse(`Path not found: ${url.pathname}`, 404, 'not_found');
    } catch (err) {
        console.error("Server Error:", err);
        return createErrorResponse(`Internal Server Error: ${err.message}`, 500, 'internal_error');
    }
  }
});
...
async function fetchTdf(logger) {
    logger.add('TDF_Fetch', 'INFO', '🚀 Starting TDF fetch...');
    ...
    if (!response.ok) {
        logger.add('TDF_Fetch', 'ERR', `Failed TDF fetch, HTTP: ${response.status}`);
        throw new Error(`Failed TDF fetch`);
    }
    ...
    try {
        ...
        logger.add('TDF_Fetch', 'ERR', `TDF non-JSON: ${responseText}`);
        throw new Error(`Invalid TDF response`);
    }

    if (!data.t) {
        logger.add('TDF_Fetch', 'ERR', `TDF missing 't' field`);
        throw new Error(`Invalid TDF response`);
    }
    ...
    // 🔥 [Fix] Clock Drift Simulation
    const fakeClockDrift = Math.floor(Math.random() * 58) + 2; 
    const humanizedTdf = realTdf + fakeClockDrift;

    logger.add('TDF_Fetch', 'SUCCESS', `✅ TDF Success (Real: ${realTdf}, Drift: ${fakeClockDrift}, Final: ${humanizedTdf})`);
    return humanizedTdf;
}
...
async function performHandshake(logger, sessionId) {
  ...
  logger.add('Handshake', 'INFO', `🤝 Using SessionID: ${sessionId.substring(0, 8)}...`);
  ...
  if (!response.ok) throw new Error(`Handshake HTTP Error: ${response.status}`);

  let data;
  try { data = JSON.parse(text); } catch(e) { throw new Error(`Handshake non-JSON: ${text.substring(0, 50)}`); }

  if (!data.success || !data.token) throw new Error(`Handshake Failed: ${JSON.stringify(data)}`);
...
// NOTE: In Bun, we don't need 'ctx'. We start the async process and return the stream immediately.
async function handleChatCompletions(request) {
  if (!verifyAuth(request)) return createErrorResponse('Authentication failed', 401, 'unauthorized');
  ...
  // Fire and forget async processing (replaces ctx.waitUntil)
  (async () => {
    let retryCount = 0;
    const MAX_RETRIES = 2;
    ...
    while (retryCount <= MAX_RETRIES) {
      ...
      try {
        logger.add('Attempt', 'INFO', `🔄 Attempt ${retryCount + 1}`);
        logger.add('Session', 'INFO', `✨ New SessionID: ${sessionId.substring(0, 8)}...`);
        ...
        if (!response.ok) {
            throw new Error(`Generation HTTP Error (${response.status}). Body: ${html}`);
        }
        ...
        if (cleanText.includes("Session ID is invalid") || cleanText.includes("Session expired") || cleanText.trim() === "") {
          throw new Error(`Invalid/Expired Session or Empty Response. Body: ${html}`);
        }

        logger.add('Generation', 'SUCCESS', 'Response received');
        ...
        logger.add('AttemptError', 'ERR', `Attempt ${retryCount + 1} Failed: ${error.message}`);
        ...
        if (retryCount < MAX_RETRIES) {
          ...
          await sendLogToClient(writer, encoder, logger.getLogs());
          const errorMsg = `\n\n[System Error]: ${error.message} (Retried ${retryCount} times)`;
...
async function handleImageGenerations(request) {
  ...
  try {
    ...
    return createErrorResponse(`Generation Failed: ${error.message}`, 500, 'image_error');
  }
}
...
function verifyAuth(request) {
  ...
  // Update: Check against CONFIG.API_KEY
  return auth && auth === `Bearer ${CONFIG.API_KEY}`;
}
...
function handleUI(request) {
  ...
  // Update: Inject CONFIG.API_KEY here
...
<body>
    <div class="sidebar">
        ...
        <div class="box">
            <div class="tabs">
                <div class="tab active" onclick="switchMode('chat')">Chat</div>
                <div class="tab" onclick="switchMode('image')">Draw</div>
            </div>
            <div id="chat-controls">
                <span class="label">Model</span>
                <select id="chat-model">${CONFIG.CHAT_MODELS.map(m => `<option value="${m}">${m}</option>`).join('')}</select>
                <span class="label">Prompt</span>
                <textarea id="chat-prompt" rows="5" placeholder="Enter content...">Hello</textarea>
            </div>
            <div id="image-controls" style="display:none">
                ...
                <span class="label">Prompt</span>
                <textarea id="image-prompt" rows="5" placeholder="Image description...">A futuristic city</textarea>
            </div>
            <button id="btn-send" onclick="handleSend()">Send Request</button>
        </div>
    ...
    <div class="main">
        <div class="chat-window" id="chat">
            <div class="msg ai">System ready (Bun Power).</div>
        </div>
    ...
    <script>
        ...
        async function handleSend() {
            ...
            clearLogs();
            appendLog({time: 'START', step: 'Client', type: 'INFO', data: 'Starting request...'});
            ...
            if (currentMode === 'chat') {
                ...
                appendMsg('user', prompt);
                const aiMsg = appendMsg('ai', 'Generating image...');
                ...
                try {
                    ...
                    appendLog({time: 'END', step: 'Client', type: 'INFO', data: 'Image generated'});
                } catch(e) {
                    aiMsg.innerText = 'Failed: ' + e.message;
                    appendLog({time: 'END', step: 'Client', type: 'ERR', data: e.message});
                }
            }
            btn.disabled = false;
